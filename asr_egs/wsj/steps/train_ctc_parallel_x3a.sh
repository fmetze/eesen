...
end_halving_inc=0        # terminate training when the accuracy improvement falls below this amount
...
[ -f $dir/.bestacc ] && bestacc=`cat $dir/.bestacc 2>/dev/null`
...
if $subsample_feats; then
  ...
  sed 's/^/0x/' $tmpdir/train0local.scp > $tmpdir/train_0local.scp
  sed 's/^/1x/' $tmpdir/train1local.scp > $tmpdir/train_1local.scp
  sed 's/^/2x/' $tmpdir/train2local.scp > $tmpdir/train_2local.scp
  paste -d '\n' $tmpdir/train_0local.scp $tmpdir/train_1local.scp $tmpdir/train_2local.scp | \
...
for iter in $(seq $start_epoch_num $max_iters); do
    ...
    # stopping criterion
    intre='^[0-9]+$'
    if [ 1 == $(bc <<< "$cvacc > $bestacc") ]; then bestacc=$cvacc; fi
    rel_impr=$(bc <<< "($cvacc-$cvacc_prev)")
    abs_impr=$(bc <<< "($cvacc-$bestacc)")
    if $halving -a [ 1 == $(bc <<< "$rel_impr < $end_halving_inc") ] \
		-a [ 1 == $(bc <<< "$abs_impr < $end_halving_inc") ]; then
    ...
    if [ 1 == $(bc <<< "$rel_impr < $start_halving_inc") -a $iter -ge $halving_after_epoch \
      -a 1 == $(bc <<< "$abs_impr < $start_halving_inc") ]; then halving=true; fi
